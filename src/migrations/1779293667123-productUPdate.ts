...
export class ProductUPdate1779293667123 implements MigrationInterface {
    name = 'ProductUPdate1779293667123'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`product_batch\` CHANGE \`is_active\` \`status\` tinyint NOT NULL DEFAULT '0'`);
        ...
        await queryRunner.query(`ALTER TABLE \`product_batch\` DROP COLUMN \`status\``);
        await queryRunner.query(`ALTER TABLE \`product_batch\` ADD \`status\` enum ('pending', 'active', 'depleted') NOT NULL DEFAULT 'pending'`);
        await queryRunner.query(`ALTER TABLE \`product_batch\` CHANGE \`activated_at\` \`activated_at\` datetime NULL`);
        await queryRunner.query(`ALTER TABLE \`product_batch\` CHANGE \`depleted_at\` \`depleted_at\` datetime NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`product_batch\` CHANGE \`depleted_at\` \`depleted_at\` datetime NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`product_batch\` CHANGE \`activated_at\` \`activated_at\` datetime NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`product_batch\` DROP COLUMN \`status\``);
        await queryRunner.query(`ALTER TABLE \`product_batch\` ADD \`status\` tinyint NOT NULL DEFAULT '0'`);
        await queryRunner.query(`ALTER TABLE \`product_batch\` ADD CONSTRAINT \`FK_0bad2a202b239f71250b7575787\` FOREIGN KEY (\`product_id\`) REFERENCES \`product\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`unit\` CHANGE \`userId\` \`userId\` int NULL DEFAULT 'NULL'`);
        await queryRunner.query(`CREATE UNIQUE INDEX \`IDX_a5483e50555eec93e3cf8d337f\` ON \`unit\` (\`name\`, \`userId\`)`);
        await queryRunner.query(`CREATE UNIQUE INDEX \`IDX_5f5ced72b895decbd4ca0318c5\` ON \`unit\` (\`label\`, \`userId\`)`);
        await queryRunner.query(`ALTER TABLE \`unit\` ADD CONSTRAINT \`FK_745632f9034cd78ed2fbea6cd1a\` FOREIGN KEY (\`userId\`) REFERENCES \`user\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`product_batch\` CHANGE \`status\` \`is_active\` tinyint NOT NULL DEFAULT '0'`);
    }
